...
$(
  `.container-fluid:eq(1) .row:eq(0) .info-box-content:eq(${Number(typeid) - 1}) .info-box-text`,
).css("color", "#e67e22");
...
$(".reservation-chunk-sub-num").text(data1.period);
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (typeid == "1")
        is = $("#ketQua").text(
          `Next Result: ${response.setting[0].wingo1 == "-1" ? "Random" : response.setting[0].wingo1}`,
        );
      if (typeid == "2")
        is = $("#ketQua").text(
          `Next Result: ${response.setting[0].wingo3 == "-1" ? "Random" : response.setting[0].wingo3}`,
        );
      if (typeid == "3")
        is = $("#ketQua").text(
          `Next Result: ${response.setting[0].wingo5 == "-1" ? "Random" : response.setting[0].wingo5}`,
        );
      if (typeid == "4")
        is = $("#ketQua").text(
          `Next Result: ${response.setting[0].wingo10 == "-1" ? "Random" : response.setting[0].wingo10}`,
        );

      if (typeid == "1")
        $("#winrate").text(
          `Next Result: ${response.setting[0].bs1 == "-1" ? "Random" : response.setting[0].bs1}`,
        );
      if (typeid == "2")
        $("#winrate").text(
          `Next Result: ${response.setting[0].bs3 == "-1" ? "Random" : response.setting[0].bs3}`,
        );
      if (typeid == "3")
        $("#winrate").text(
          `Next Result: ${response.setting[0].bs5 == "-1" ? "Random" : response.setting[0].bs5}`,
        );
      if (typeid == "4")
        $("#winrate").text(
          `Next Result: ${response.setting[0].bs10 == "-1" ? "Random" : response.setting[0].bs10}`,
        );
    },
  });
});
